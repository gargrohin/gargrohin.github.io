...
This post covers two experiments where I designed rewards that seemed reasonable, trained agents on them, and watched them optimize for exactly the wrong thing. One in discrete text generation with REINFORCE, one in continuous robotic control with PPO. The lessons were the same.
...
The training curve tells the full story. Reward spikes to ~1.0 early on as the model learns to spam sentiment-loaded words ("good good good", "bad bad bad"). Then it collapses as the outputs degenerate into neutral repetitions like "ClearClear" that the classifier scores either way.

### Analysis

The reward measured one property: classifier agreement. It captured nothing about fluency, coherence, or relevance to the prompt. With 50,257 tokens to choose from at each step and only a binary reward at the end, the optimizer found a hack.

This is **Goodhart's Law** in RL: when a proxy metric (classifier score) is optimized directly, it stops measuring what you actually care about.

### Fix Attempt: KL Divergence Constraint

I tried to constraining the policy to stay close to the pretrained model:
...
I love robotics. I don't know exactly why but there's something about enabling an inorganic object to understand me and the world, and making it act **autonomously**. It's hard enough putting this objective into words and math, let alone actually do it.

So to keep things simple, I started with Robosuite, a robotics benchmark built on MuJoCo physics. It provides standard manipulation tasks with consistent APIs, so I could focus on reward design and RL training instead of environment plumbing.

**Task**: A 7-DOF Panda robot arm must pick up a cube and lift it 4cm above the table (robosuite Lift environment, 500 timesteps per episode). The policy sees joint positions, gripper state, and object positions as a flat vector, and outputs continuous joint velocity targets plus a gripper command.
...
The **per-step component dominates**. Hovering near the cube for 500 steps earns 200. Completing the task at step 100 earns 42.5. The optimal policy is to never finish.
...
**Result**: 40% success rate. Progress! But evaluating the policy revealed the same problem in a different form. Failed episodes had higher rewards than successful ones:
...
The training curve looks like progress but it's really not. The agent learned to grasp and hold, collecting +0.5/step for the entire episode. Reward goes up because it's getting better at holding, not at lifting.

![Reward climbing to ~75-150: looks like learning, but the agent is just holding the cube](/assets/images/28asm_slowclimb.png)

Notice the y-axis is 0 to 150+, all positive, because every reward component is positive. The agent is rewarded just for existing near the cube.

![Robot grasping and holding cube without lifting](/assets/images/iter1_grasp_hold.gif)

### Iteration 2: Time Penalty

I thought the fix was obvious: add a per-step penalty to prevent duration exploitation, and increase the success bonus:
...
![Flat line at -250 for the entire training run](/assets/images/robo-flatline.png)

Compare the y-axis to the previous chart: it went from 0-150 to -140 to -250. The time penalty flipped the entire reward landscape negative. The -0.5/step penalty over 500 steps gives -250, and the agent can't earn enough positive reward to offset it.
...
Investigating further, I realized the reaching reward formula itself was a hidden problem:

```python
reaching = weight * (1 - tanh(coeff * distance))
```

With `coeff=10`, `tanh` saturates almost immediately. For any distance beyond ~0.3m, the reaching reward is essentially zero. The agent only "feels" the reward when already very close to the cube, but random exploration *rarely* gets that close.
...
![tanh reaching reward: coeff=10 is flat by 0.3m, coeff=3 still has gradient](/assets/images/tanh_curves.png)

At the typical starting distance (~0.3m), coeff=10 gives essentially zero reward. The agent has no signal to learn from. Coeff=3 still has meaningful gradient at that distance.

Reducing the coefficient to 3 spreads the gradient over a larger distance. Combined with a lighter time penalty (0.1) and higher success reward (200), the reward landscape now has the right ordering:
...
The ordering is finally right. The agent occasionally completes the task, but the policy is unstable. One eval run gives 20% success, the next gives 0%. Four iterations of reward shaping, and the best I got is a policy that sometimes works.
...
Plotting the reward landscape before training would have caught this. I built a visualization notebook after three failed runs. It should have been the first thing I did.

### 3. Evaluate What the Agent Does, Not What It Scores
